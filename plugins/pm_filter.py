...
from database.ia_filterdb import col, sec_col, db as asldb, sec_db, get_file_details, get_search_results, get_bad_files
...
from AnimeSLinkzZ.util.file_properties import get_name, get_hash, get_media_file_size
     ...
     #   if not search:
      ...
      #      return
        ...
        btn2 = [[
            ...
            InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/AnimeSLinkzZ")
        ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Sᴏᴜʀᴄᴇ Cᴏᴅᴇ', url="https://github.com/VJBots/AnimeSLinkzZ-FILTER-BOT")
        ],[
        ...
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/AnimeSLinkzZ")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        ...
        for AnimeSLinkzZ in movienamelist:
            try:
                mv_rqst = mv_rqst.capitalize()
            except:
                pass
            if mv_rqst.startswith(AnimeSLinkzZ[0]):
                await auto_filter(client, AnimeSLinkzZ, msg, reply_msg, vj_search_new)
                break
        reqst_gle = mv_rqst.replace(" ", "+")
        button = [[
            InlineKeyboardButton("Gᴏᴏɢʟᴇ", url=f"https://www.google.com/search?q={reqst_gle}")
