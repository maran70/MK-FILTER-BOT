# Don't Remove Credit Tg - @AnimeSLinkzZ
# Subscribe YouTube Channel For Amazing Bot https://youtube.com/@AnimeSLinkzZ
# Ask Doubt on telegram @AnimeSLinkzZ

# Clone Code Credit : YT - @AnimeSLinkzZ / TG - @AnimeSLinkzZ / GitHub - @VJBots
...
async def clone_menu(client, message):
    ...
    if await db.is_clone_exist(message.from_user.id):
        ...
        pass
    AnimeSLinkzZ = await client.ask(message.chat.id, "<b>1) sᴇɴᴅ <code>/newbot</code> ᴛᴏ @BotFather\n2) ɢɪᴠᴇ ᴀ ɴᴀᴍᴇ ꜰᴏʀ ʏᴏᴜʀ ʙᴏᴛ.\n3) ɢɪᴠᴇ ᴀ ᴜɴɪǫᴜᴇ ᴜsᴇʀɴᴀᴍᴇ.\n4) ᴛʜᴇɴ ʏᴏᴜ ᴡɪʟʟ ɢᴇᴛ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ʏᴏᴜʀ ʙᴏᴛ ᴛᴏᴋᴇɴ.\n5) ꜰᴏʀᴡᴀʀᴅ ᴛʜᴀᴛ ᴍᴇssᴀɢᴇ ᴛᴏ ᴍᴇ.\n\n/cancel - ᴄᴀɴᴄᴇʟ ᴛʜɪs ᴘʀᴏᴄᴇss.</b>")
    if AnimeSLinkzZ.text == '/cancel':
        await AnimeSLinkzZ.delete()
        return await message.reply('<b>ᴄᴀɴᴄᴇʟᴇᴅ ᴛʜɪs ᴘʀᴏᴄᴇss 🚫</b>')
    if AnimeSLinkzZ.forward_from and AnimeSLinkzZ.forward_from.id == 93372553:
        try:
            bot_token = re.findall(r"\b(\d+:[A-Za-z0-9_-]+)\b", AnimeSLinkzZ.text)[0]
        except:
    ...
    try:
        vj = Client(
            ...
            bot_token=bot_token,
            plugins={"root": "CloneAnimeSLinkzZ"}
        )
        await vj.start()
        bot = await vj.get_me()
        ...
        await msg.edit_text(f"⚠️ <b>Bot Error:</b>\n\n<code>{e}</code>\n\n**Kindly forward this message to @AnimeSLinkzZ to get assistance.**")
